...
class FrameInterpolater():
    ...
    def parse_key_frames(self, string, filename='unknown'):
        ...
        for match_object in string.split(","):
            ...
            s = self.seed
            try:
                sanitized_value = self.sanitize_value(frameParam[0].strip())
                if check_is_number(sanitized_value):
                    value = float(sanitized_value)
                    if 0 <= value <= 1:
                        frame = round(value * max_f)
                    else:
                        frame = int(value)
                else:
                    evaluated_value = numexpr.evaluate(sanitized_value.replace("'", "", 1).replace('"', "", 1)[::-1].replace("'", "", 1).replace('"', "", 1)[::-1])
                    if 0 <= evaluated_value <= 1:
                        frame = round(evaluated_value * max_f)
                    else:
                        frame = int(evaluated_value)
                frames[frame] = frameParam[1].strip()
            except SyntaxError as e:
                e.filename = filename
                raise e
            except ValueError as e:
                raise RuntimeError(f"Invalid value for frame number: {frameParam[0]}") from e
